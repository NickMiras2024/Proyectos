...
const consultaUsuariosPreInsert = 'insert into usuariosPed (nombre,Email,rango,tel,fechaI,DNI,contrasena) VALUES (?,?,?,?,?,?,?)';
...
const consultaEntrenamientosInsert = 'insert into entrenamientos (UsuarioDNI,dia,hs,entrenamientos,urlPDF) values (?,?,?,?,?)';
...
function insertarDatosUsuariosPed(valores) {
  return new Promise((resolve, reject) => {
    if (valores.length == 7) {
      try {
        obtenerDB().run(consultaUsuariosPreInsert, valores, (err) => {
          if (err) {
            reject(err);
          } else {
            resolve('Inserción exitosa');
...
module.exports = {insertarDatosUsuariosPed ,obtenerDB, obtenerDatosTabla, insertarDatosHistorial, insertarDatosUsuarios, insertarDatosEntrenamientos, eliminarUsuario, enviarQuerysP, eliminarcliente }
