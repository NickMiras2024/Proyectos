...
class Neurona():
    def __init__(self, cantidad_de_pesos, tasa, presicion, funcion, pesos=None, sesgo=None):
        ...
        self.sesgo = sesgo if sesgo is not None else np.random.uniform(-0.1, 0.1)
        self.pesos = pesos if pesos is not None else [np.random.uniform(-0.1, 0.1) for _ in range(cantidad_de_pesos)]
