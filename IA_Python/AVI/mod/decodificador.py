...
import numpy as np
import cross_attention as CRAT
...
class Decodificador():
    def __init__(self,
        ...
        for cabeza in range(cantidad_de_cabezas_SLFA):
            ...
            for i in range(len(rutas)):
                ...
                print(rutas_finales)
            self.cross_cabezas.append(CRAT.Cross_attention(
                            rutas_finales,
        ...
        #addYnorm0
        self.addYnomr0 = AN.LayerNorm(dimencion_del_embeding,1e-5,ruta_ADD_NORM_0)
        ...
        print(self.cabezas,self.cross_cabezas,self.addYnomr1,self.feedFordward,self.addYnomr2)

    #acá frases tiene que ser una matriz 2D
    def calculo_hacia_adelante(self,frase,matriz_de_encoder):
        ...
        for cabeza in range(len(self.cabezas)):
            ...
            print(resultado_de_cabeza)
            resultado_de_las_cabezas.append(resultado_de_cabeza)

        #concatenacion de las semi_palabras
        palabras_completas_con_atencion = []

        for resultado in range(len(resultado_de_las_cabezas)):
            for semi_palabra in range(len(resultado_de_cabeza[resultado])+1):
                if resultado == 0:
                    palabras_completas_con_atencion = resultado_de_las_cabezas[resultado]
                    print('\nPC', palabras_completas_con_atencion)
                    break
                else:
                    print(semi_palabra,len(resultado_de_las_cabezas[resultado]))
                    palabras_completas_con_atencion[semi_palabra] = palabras_completas_con_atencion[semi_palabra] + resultado_de_las_cabezas[resultado][semi_palabra]
        ...
        #___________________Cross_attention__________________:
        resultado_de_las_cross_cabezas = []

        for cabeza in range(len(self.cross_cabezas)):
            resultado_de_cabeza = self.cross_cabezas[cabeza].utilizar_CF(frase,matriz_de_encoder)
            print(resultado_de_cabeza)
            resultado_de_las_cross_cabezas.append(resultado_de_cabeza)

        #concatenacion de las semi_palabras
        palabras_completas_con_cross_atencion = []

        for resultado in range(len(resultado_de_las_cross_cabezas)):
            for semi_palabra in range(len(resultado_de_las_cross_cabezas[resultado])):
                if resultado == 0:
                    palabras_completas_con_cross_atencion = resultado_de_las_cross_cabezas[resultado]
                    print('\nPC', palabras_completas_con_cross_atencion)
                    break
                else:
                    print(semi_palabra,len(resultado_de_las_cross_cabezas[resultado]))
                    palabras_completas_con_cross_atencion[semi_palabra] = palabras_completas_con_cross_atencion[semi_palabra] + resultado_de_las_cross_cabezas[resultado][semi_palabra]
        
        print('\nPC', palabras_completas_con_cross_atencion)

        #__________________addYNomr_Cross______________

        #suma
        resultado_de_add_0 = self.addYnomr0.add(palabras_completas_con_cross_atencion,resultado_de_normalizacion_1)

        #normalizado
        resultado_de_normalizacion_0 = self.addYnomr0.normalizado(resultado_de_add_0)
        ...
        for palabra in range(len(resultado_de_normalizacion_0)):
        ...
        #suma
        resultado_de_add_2 = self.addYnomr2.add(palabras_procesadas,resultado_de_normalizacion_0)
    ...
    def backpropagation(self,errores):
        ...
        objeto_con_errores_para_cross = {}
        palabras_errores_para_cross = {}
        objeto_con_errores_para_cross_cod = {}
        palabras_errores_para_cross_cod = {}
        ...
        #___________________FEEDFORDWARD____________________
            ...
            errores_para_add_y_norm0 = self.feedFordward.backpropagation_intrared(error_para_feedFordward,palabra)
            
       
        #___________________addYNormCross_______________________
            # print('___________________addYNormCross_______________________\n')
            vector_de_error_para_add_y_norm0 = self.sumador_de_columnas(errores_para_add_y_norm0)
            
            vector_de_error_para_add_y_norm0_np = np.array(vector_de_error_para_add_y_norm0)
            error_para_cross_attention = self.addYnomr2.backward(vector_de_error_para_add_y_norm0_np)

        #___________________CROSS_ATTENTION________________________    
            # print('___________________CROSS_ATTENTION________________________\n')

            errores_para_cada_una_de_las_cabezas_cross = {}

            #se separa el error de columpnas para enviarselas a las respectivas cabezas
            for cabeza in range(len(self.cross_cabezas)):
                # print(error_para_self_attention)
                errores_cf = []
                for fila in range(len(error_para_cross_attention)):
                    vector = []
                    for col in range(int(len(error_para_cross_attention[0])/len(self.cabezas))):
                        # print('hola',len(self.cabezas),int(len(error_para_self_attention[0])/len(self.cabezas)),col)
                        vector.append(error_para_cross_attention[fila][col+len(self.cabezas)*cabeza])
                    errores_cf.append(vector)
                errores_para_cada_una_de_las_cabezas_cross[cabeza] = errores_cf



            #se entrenan las cabezas con los errores  
            for cabeza in range(len(self.cross_cabezas)):
                vector_de_error_para_cross_attention_cabeza = self.sumador_de_columnas(errores_para_cada_una_de_las_cabezas_cross[cabeza])
                error_para_capa_anterior,error_para_decoder = self.cross_cabezas[cabeza].backpropagation(vector_de_error_para_cross_attention_cabeza,palabra)

               #se guarda en el objeto de errores los errores que devolvieron cada una de las cabezas 
                objeto_con_errores_para_cross[cabeza] = error_para_capa_anterior
                objeto_con_errores_para_cross_cod[cabeza] =  error_para_decoder
            ...
            for numero_de_cabeza in range(len(objeto_con_errores_para_cross)):
                for numero_de_palabra in range(len(objeto_con_errores_para_cross[numero_de_cabeza])):
                    if numero_de_cabeza == 0:
                        palabras_errores_para_cross = objeto_con_errores_para_cross[numero_de_cabeza]
                        break
                    else: 
                        for fila in range(len(objeto_con_errores_para_cross[numero_de_cabeza][numero_de_palabra])):
                            palabras_errores_para_cross[numero_de_palabra].append(objeto_con_errores_para_cross[numero_de_cabeza][numero_de_palabra][fila])


            #convierte el error del codificador (que en un inicio era del tamaño de las cabezas) en uno solo para que se pueda enviar a la capa anterior
            for numero_de_cabeza in range(len(objeto_con_errores_para_cross_cod)):
                for numero_de_palabra in range(len(objeto_con_errores_para_cross_cod[numero_de_cabeza])):
                    if numero_de_cabeza == 0:
                        palabras_errores_para_cross_cod = objeto_con_errores_para_cross_cod[numero_de_cabeza]
                        break
                    else: 
                        for fila in range(len(objeto_con_errores_para_cross_cod[numero_de_cabeza][numero_de_palabra])):
                            palabras_errores_para_cross_cod[numero_de_palabra].append(objeto_con_errores_para_cross_cod[numero_de_cabeza][numero_de_palabra][fila])


        for PL in range(len(palabras_errores_para_cross)):
                
            #___________________addYNorm1________________________
                # print('___________________addYNorm1________________________\n')
                
                # MDM.mostrar_matriz(palabras_errores_para_cross[PL])

                vector_de_error_para_add_y_norm1 = self.sumador_de_columnas(palabras_errores_para_cross[PL])
        ...
        return palabras_errores,palabras_errores_para_cross_cod
