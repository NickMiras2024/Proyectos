...
carpeta_nombre = os.path.join(ruta_actual,'data_2')
...
texto_para_el_entrenamiento = """<usr> El perro <bot>dormía tranquilo en el sofá.
<usr> Una tarde <bot>lluviosa cubría todo el campo.
<usr> La luna <bot>brillaba sobre el mar en calma. 
<usr> En el <bot>bosque se escuchaban ruidos extraños. 
<usr> Un avión <bot>pasó muy alto entre las nubes. 
<usr> El código <bot>tenía errores en varias líneas. 
<usr> Los robots <bot>funcionan con electricidad y lógica. 
<usr> Ella tomó <bot>el cuaderno y salió del aula. 
<usr> Las ideas <bot>fluyen mejor cuando hay silencio. 
<usr> En la <bot>mesa había papeles y lápices sueltos. 
<usr> Los autos <bot>avanzaban despacio por la avenida. 
<usr> Un libro <bot>contaba historias de dragones antiguos. 
<usr> Algunas veces <bot>el sistema falla por sobrecarga. 
<usr> El sol <bot>salía entre las montañas nevadas. 
<usr> Mi amigo <bot>construyó un robot con cartón. 
"""
...
capa_embeding = EMB.Crear_capa_embeding(embeding_ruta,dimencion_del_embeding,cantidad_de_palabras_en_el_diccionario,TDA,leaky_relu_function)
...
cantidad_de_neuronas1 = 64
cantidad_de_neuronas2 = 128
...
def entrenar(objeto_pre_res):
    ...
    error_promedio_de_datos_completos = 0
    ...
    for numero_de_palabra_de_respuesta in range(len(respuesta_divida)):
        ...
        resultado_del_bloque_1 = bloque_1.calculo_hacia_adelante(matriz_de_embeding)
        ...
        vector_de_errores= []
        vector_de_errores.append(resultado_final)

        # Encontramos el índice de la palabra correcta
        indice_correcto = vector_de_respuestas_esperadas.index(1)

        # Guardamos la probabilidad que el modelo le asignó a esa palabra
        probabilidad_correcta = resultado_final[indice_correcto]

        # Sumamos esa pérdida para visualizar luego
        error_promedio += -np.log(probabilidad_correcta + 1e-12)
        # (+1e-12 para evitar log(0) si la red dio 0 por accidente)


        #guardar_error_por_palabra
        with open('muestreo_de_errores_Recientes_palabra.txt','a')as archivo:
            archivo.write(f'{ -np.log(probabilidad_correcta + 1e-12) },')




        vector_de_errores[0][indice_correcto] -= 1
        ...
        error_para_el_embeding = bloque_1.backpropagation(objeto_para_bloque1)
    ...
    error_promedio_de_datos_completos += error_promedio / len(respuesta_divida)
    return respuesta_de_red,error_promedio_de_datos_completos
...
for i in range(espoch):
    ...
    for dato in range(len(datos_para_entrenar)):
        if conteo == 1: 
            ...
            conteo = 0
        res,error_de_pasada = entrenar(datos_para_entrenar[dato])
        error_promedio_de_datos_completos += error_de_pasada
        print(res)
        print(datos_para_entrenar[dato]['respuesta'])
        print(bloque_1.mostrar_data())
        with open('muestreo_de_errores_Recientes.txt','a')as archivo:
            archivo.write(f'{ error_de_pasada },')

        print('_____________________________________________')
        conteo+= 1
    print('___________ERROR_PROMEDIO_DE_DATOS_COMPLETOS________________')
    print(error_promedio_de_datos_completos / len(datos_para_entrenar) )
    with open('muestreo_de_errores.txt','a')as archivo:
        archivo.write(f'{error_promedio_de_datos_completos / len(datos_para_entrenar)},')
    print(i ,'de',espoch-1)
